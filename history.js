...
function createTabItem(tab) {
  ...
  if (tab.favIconUrl) {
    favicon.innerHTML = `<img src="${tab.favIconUrl}" alt="" width="24" height="24">`;
  } else {
...
async function switchToTab(tabId) {
  try {
    ...
    if (tab && !tab.discarded) {
      ...
      // Close this history tab
      await chrome.tabs.query({ active: true, currentWindow: true });
      const [currentTab] = await chrome.tabs.query({ url: 'chrome-extension://*/*' });
      if (currentTab && currentTab.url.includes('history.html')) {
...
// Event listener: ESC key to close
